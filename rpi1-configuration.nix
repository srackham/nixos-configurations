{...}: {
  ...
  services.cron = {
    ...
    systemCronJobs = [
      ...
      "45 */8 * * * mount /dev/disk/by-label/BACKUPS /media/backups && rsync -aH --delete /files/backups /media/backups >/dev/null && umount /media/backups && logger -t data-backup -p user.info 'Backup rpi1:/backups to USB drive completed successfully' || logger -t data-backup -p user.err 'Backup rpi1:/backups to USB drive failed'"
